...
def _get_filename(path, init_year, member, variable):
    ptn = path + "/" + variable + "/*" + str(init_year) + "*" + str(member).zfill(3) + "*" + variable + "*.nc"
    f = glob.glob(ptn)
    if len(f) != 1:
        raise
    return f[0]

def _compute_djfm(x, init_year, start_year=2, end_year=9): #, varname, init_year, start_year=2, end_year=9):
...
def _get_output_filename(init_year, member, project = "CMIP6", model = "CESM1-1-CAM5-CMIP5"):
    fn = project + "_" + model + \
        "_Amon_dcppA-hindcast_s" + \
        str(init_year) + "-" + "r" + str(member) + "i1p1f1" + \
        "_pr_gn_" + str(init_year + 1) + \
        "-" + str(init_year + 8) + ".nc"
    return fn
...
def main(config):
    ...
    european_precip_outdir = os.path.join(output_dir, "recipe1/work/european_precip/european_precip")
    ...
    for i in range(len(init_years)):
        init_year = init_years[i]; print(init_year)
        for j in range(len(members)):
            ...
            for k in range(len(variables)):
                ...
                iris.coord_categorisation.add_season(ds, 'time', seasons = ['djfm', 'am', 'jjas', 'on'])
                iris.coord_categorisation.add_season_year(ds, 'time', seasons = ['djfm', 'am', 'jjas', 'on'])
